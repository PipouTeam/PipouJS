...
describe('GET /api/relation-types', () => {
    it('devrait retourner la liste (public)', async () => {
        await pool.query("INSERT INTO relation_types (name) VALUES ('Soi'), ('Famille')");

        const res = await request(app).get('/api/relation-types');

        expect(res.status).toBe(200);
        expect(res.body.relation_types).toHaveLength(2);
    });
});

describe('POST /api/relation-types', () => {
    it('devrait creer un type de relation (admin)', async () => {
        ...
        expect(res.body.relation_type.name).toBe('Amis');
    });
...
describe('PUT /api/relation-types/:id', () => {
    it('devrait modifier un type de relation (admin)', async () => {
        const admin = await createAdminAndLogin(app, pool);
        const rt = await pool.query("INSERT INTO relation_types (name) VALUES ('Ancien') RETURNING id");

        const res = await request(app)
            .put(`/api/relation-types/${rt.rows[0].id}`)
            .set('Authorization', `Bearer ${admin.token}`)
            .send({ name: 'Nouveau' });

        expect(res.status).toBe(200);
        expect(res.body.relation_type.name).toBe('Nouveau');
    });
...
describe('DELETE /api/relation-types/:id', () => {
    it('devrait supprimer un type de relation (admin)', async () => {
        const admin = await createAdminAndLogin(app, pool);
        const rt = await pool.query("INSERT INTO relation_types (name) VALUES ('Suppr') RETURNING id");

        const res = await request(app)
            .delete(`/api/relation-types/${rt.rows[0].id}`)
            .set('Authorization', `Bearer ${admin.token}`);

        expect(res.status).toBe(200);

        const check = await pool.query('SELECT * FROM relation_types WHERE id = $1', [rt.rows[0].id]);

<template>
  <div>
    <div class="d-flex justify-space-between align-center mb-4">
      <h1 class="text-h4">Types de ressources</h1>
    ...
    <v-dialog v-model="dialog" max-width="400">
      <v-card>
        <v-card-title>{{ editMode ? 'Modifier' : 'Ajouter' }} un type de ressource</v-card-title>
        <v-card-text>
...
const fetchItems = async () => {
  loading.value = true
  try {
    const { data } = await axios.get('/api/resource-types')
    items.value = data.resource_types || []
  } catch (e) {
...
const save = async () => {
  ...
  try {
    if (editMode.value) {
      await axios.put(`/api/resource-types/${form.value.id}`, { name: form.value.name })
      showSnackbar('Type de ressource mis à jour')
    } else {
      await axios.post('/api/resource-types', { name: form.value.name })
      showSnackbar('Type de ressource créé')
    }
    dialog.value = false
    fetchItems()
  } catch (e) {
...
const deleteItem = async (item) => {
  if (!confirm('Voulez-vous vraiment supprimer ce type de ressource ?')) return
  try {
    await axios.delete(`/api/resource-types/${item.id}`)
    showSnackbar('Type de ressource supprimé')
    fetchItems()
  } catch (e) {
